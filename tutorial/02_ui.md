...
* 8.3 The same strategy can be used to add the button but instead add them inside the toolbar.
...
* 9.4 Add a save button to the form.
* 9.5 Connect buttons with outcomes by setting the action on the button.
...
* 9.4 You can add commit/rollback buttons by clicking the "submit" option in the form dialog.
* 9.5 You need to change the button to call your actions inetad of the default actions (which is rollbak/commit)
 
## 10.0 Add routing between views.
...
* 10.1 Create routing between the view by updating your controller flow.
* 10.2 Add routing rules from the view form to the edit task flow.
* 10.3 Also add routing back from the edit dialog

### Hints:

* 10.1 The default unbounded taskflow is called adfc-config.xml
* 10.2 Drag the employees view and the task flow (notice that the second view cannot be run independently) to this task flow.
* 10.3 Dont forget to add a return action
* 10.3 Since we already have commit and rollback we can rollback the global transaction (or commit it)

### Question:
 * 10.1 Why is the design of the commit/rollback we have bad?
 * 10.2 What would be a better design?

## 11.0 Add edit button to edit employees.
...
* 11.1 Add an edit button to the list view
* 11.2 Bind the button to the edit action in the task flow.

### Hints:

* 11.1 Surround the table with a "Panel Collection"
* 11.1 Add a toolbar with a button to the new toolbar facet.
* 11.3 Set the outcome of the action to editEmployee
* 11.3 Once you start to reorganize your form your partial triggers will show up as red. This is due to the ::: actually having a meaning and not beeing random.

### Questions:

* 11.1 What is the meaning of :: in the ids?
...
* 13.1 Change the gravatar field in the table and form on the employees page to display the photo.
* 13.2 The image in the table is to large. Make it smaller
* 13.3 Image in form is to small, make it bigger

### Hints:

* 13.1 Use an image to display the image
* 13.1 Set the source to the el expression you used for output before.

### Questions:

* 13.1 Why is the one called row.Gravatar and the other bindings.Gravatar.inputValue
* 13.1 Why is my solution to shrinking the image better than yours?

## 14.0 Change the task flow to support creating users.
...
* 14.1 Add a parameter (action) to the task flow. 
* 14.2 Then extend the flow with a routing component which routes based on the action to either "the old edit" flow.
* 14.3 Or a new createinsert method call.

### Hints:

* 14.1 Add an argument (action) to the task flow.
* 14.2 Add an inbound router which routes given the parameter
* 14.3 Route the create leg to a "create new"
* 14.4 The action parameter can be found in the #{pageFlowScope.action}
* 14.5 The createInsert new operation can be dragged from the data control (use the correct view object)
* 14.6 *DONT* forget to change the default activity to the router (I did).
* 14.7 If you get "activity not found" verify that ALL required attributes have been set (for instance default outcome on the router)

### Questions:

* 14.1 Why do we use the "strange" expression (and not: #{pageFlowScope.action} == 'edit'
* 14.1 Why do we need the router and the parameter? (why not two starts?)

## 15.0 Add a managed bean to store the variable between two views.
...
* 15.1 Add a managed bean to the faces-config.xml
* 15.2 Add the SAME managed bean to the task flow (this will remove it from the faces-config)
* 15.3 Add a "parameter" to the bean called action
* 15.4 Configure the input parameters for the task flow to use the bean parameters

### Hints:

* 15.1 Adding a managed bean is done in the unbounded taskflow (adfc-config.xml)
* 15.2 Adding managed beans is a bit strange if you add them to the faces-config.xml file and then move them adfc-config.xml it works automagically) if not you have to create the class manually. (best option for the class is to check the git repo).
* 15.3 Add a single string parameter (action) and generate accessors.
* 15.4 Input parameters can be configure in the unbounded taskflow for the bounded task flow.
* 15.5 If you can not find the bean in the expression dialog you have not configured it

### Questions:

* 15.1 Why do we need a managed bean? (What other options do we have?)
* 15.2 Why do we use the request scope?
* 15.3 Whats the difference between adding the bean to the unbounded taskflow and the faces-config?

## 16.0 Add "new" button and connect them.
...
* 16.1 Add the new button like before (same action)
* 16.2 Change the value of the bean with an action listsner
* 16.3 The Set property action listsner is suitable for this

### Questions:

* 16.1 Why dont we have to add "" around the create/edit values?
* 16.2 How can we se the value in "runtime" if we want to debug the flow?
* 16.3 What happens if we set the value to "foobar"?
